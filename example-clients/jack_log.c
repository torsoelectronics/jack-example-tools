...
disk_thread (void *arg)
{
	...
	while (1) {
      ...
      for (int i = 0; i < samples_per_frame; i++) {
        fprintf(info->file, "%f, ", *((float*)(framebuf + i * sample_size)));
      }
      fprintf(info->file, "\n");
...
setup_disk_thread (jack_thread_info_t *info)
{
  ...
  if ((info->file = fopen(info->path, "w")) == NULL) {
...
main (int argc, char *argv[])
{
	...
	if (show_usage || thread_info.path == NULL || optind == argc) {
		fprintf (stderr, "usage: jack_log -f filename [ -d second ] [ -B bufsize ] port1 [ port2 ... ]\n");
		exit (1);
	}
